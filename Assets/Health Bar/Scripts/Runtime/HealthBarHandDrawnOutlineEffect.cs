...
namespace HandDrawnHealthBar
{
    ...
    public sealed class HealthBarHandDrawnOutlineEffect : BaseMeshEffect
    {
        private const int OutlineSampleCount = 8;

        [SerializeField]
        private Color outlineColor = new(0.035f, 0.03f, 0.025f, 1f);
        ...
        public override void ModifyMesh(VertexHelper vertexHelper)
        {
            ...
            int variation = PositiveModulo(GetAnimationFrame(), variationCount);
        ...
        private Vector2 CalculateOutlineOffset(int sampleIndex, int variation)
        {
            ...
            float animatedAngle =
                baseAngle +
                Mathf.Sin(seed + variation * 1.73f + sampleIndex) * 0.08f;
            float irregularity =
                1f +
                Mathf.Sin(seed * 0.37f + variation * 2.41f + sampleIndex * 1.91f) * 0.14f;

            return new Vector2(Mathf.Cos(animatedAngle), Mathf.Sin(animatedAngle)) *
                   thickness *
                   irregularity;
        }
